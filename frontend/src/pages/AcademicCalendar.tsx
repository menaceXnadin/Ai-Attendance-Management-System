...
const CustomDateCellWrapper = (props: DateCellWrapperProps) => {
  ...
  if (events) {
    ...
    if (event) {
      const color = (event.resource && typeof event.resource === 'object' && 'color_code' in event.resource)
        ? (event.resource as { color_code?: string }).color_code
        : undefined;
      bg = color || '#8B5CF6';
    }
  }
  return (
    <div style={{ background: bg, width: '100%', height: '100%', minHeight: 32, position: 'relative' }}>
      {children}
    </div>
  );
};
...
const AcademicCalendar: React.FC = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filteredEvents = data.filter(event => {
          ...
          // Include holidays (including Saturday holidays)
          if (event.event_type === 'HOLIDAY' || event.event_type === 'holiday' ||
              (event.title && event.title.toLowerCase().includes('holiday'))) {
            console.log('✅ Including holiday:', event.title);
            return true;
          }
          
          // Include exams
          if (event.event_type === 'EXAM' || event.event_type === 'exam') {
            console.log('✅ Including exam:', event.title);
            return true;
          }
          
          // Include special events
          if (event.event_type === 'SPECIAL_EVENT' || event.event_type === 'special_event') {
            console.log('✅ Including special event:', event.title);
            return true;
          }
          
          // Include cancelled classes (administrative announcements)
          if (event.event_type === 'CANCELLED_CLASS' || event.event_type === 'cancelled_class') {
            console.log('✅ Including cancelled class:', event.title);
            return true;
          }
          
          // Include CLASS events - these represent college opening days, not individual classes
          if (event.event_type === 'CLASS' || event.event_type === 'class') {
            console.log('✅ Including CLASS event (college opening day):', event.title);
            return true;
          }
          
          // Exclude only specific individual subject classes based on title patterns
          // These are course-specific and not for general academic calendar
          if (event.title && (
            ...
            /^[A-Za-z\s]+\d+(\s+(Class|Lesson))?$/.test(event.title) ||
            // Also check for specific subject-based patterns
            (event.subject_id && (
              event.title.toLowerCase().includes('lecture') ||
              event.title.toLowerCase().includes('tutorial') ||
              event.title.toLowerCase().includes('practical')
            ))
          )) {
            console.log('❌ Excluding individual subject class:', event.title);
            return false;
          }
          
          console.log('✅ Including by default:', event.title, 'Type:', event.event_type);
  ...
  // Handle event click
  const handleEventClick = (event: { resource: CalendarEvent }) => {
  ...
  const getEventTypeInfo = (type: string) => {
    const types: { [key: string]: { icon: LucideIcon; color: string; label: string } } = {
      class: { icon: BookOpen, color: '#22C55E', label: 'Event' },
  ...
  const getCleanTitle = (title: string, eventType: string) => {
    if (eventType === 'class' && title.endsWith(' Class')) {
  ...
  // Custom day cell styling for holidays
  const dayPropGetter = (date: Date) => {
    const dateStr = moment(date).format('YYYY-MM-DD');
    
    // Check if this date has any holiday events
    const hasHoliday = events.some(event => 
      event.start_date === dateStr && 
      (event.event_type === 'holiday' || event.title?.toLowerCase().includes('holiday'))
    );
    
    // Check if it's a Saturday
    const isSaturday = moment(date).day() === 6;
    
    if (hasHoliday || isSaturday) {
      return {
        style: {
      ...
      <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="cancelled_class">Cancelled</option>
                </select>
        ...
        <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div style={{ height: '600px' }} className="calendar-dark">
              <style>
                {`
                  /* Force override all calendar styles with maximum specificity */
                  .calendar-dark .rbc-calendar .rbc-event,
                  .calendar-dark .rbc-calendar .rbc-month-view .rbc-event,
                  .calendar-dark .rbc-calendar .rbc-week-view .rbc-event,
                  .calendar-dark .rbc-calendar .rbc-day-view .rbc-event {
                  ...
                  .calendar-dark .rbc-calendar .rbc-event:hover {
                    transform: translateY(-2px) scale(1.02) !important;
                    box-shadow: 0 12px 40px rgba(0, 0, 0, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.3) !important;
                  }
                  
                  /* Modern event content styling with maximum specificity */
                  .calendar-dark .modern-event {
                    width: 100% !important;
                  ...
                  .calendar-dark .event-gradient-overlay {
                  ...
                  .calendar-dark .event-header {
                    display: flex !important;
                    align-items: center !important;
                    gap: 6px !important;
                  }
                  
                  .calendar-dark .event-icon {
                  ...
                  .calendar-dark .time-icon {
                    font-size: 10px !important;
                    opacity: 0.8 !important;
                  }
                  
                  /* Animated shine effect */
                  .calendar-dark .event-shine {
                    position: absolute !important;
                    top: -50% !important;
                    left: -50% !important;
                    width: 200% !important;
                  ...
                  .calendar-dark .rbc-event:hover .event-shine {
                    transform: translateX(100%) translateY(100%) !important;
                  }
                  
                  /* Override default event styles completely */
                  ...
                  /* Modern Saturday cell styling - simplified */
                  ...
                  /* Style all Saturday cells in month view - simplified */
                  ...
                  /* Clean "Holiday" text centered - red to match theme */
                  .rbc-month-view .rbc-date-cell:nth-child(7n)::after {
                  ...
                  /* Saturday date styling - simplified */
                  .rbc-month-view .rbc-date-cell:nth-child(7n) .rbc-button-link {
                    color: #EF4444 !important;
                    font-weight: 600 !important;
                  }
                  
                  /* Don't hide event containers for Saturday cells */
                  .rbc-month-view .rbc-date-cell:nth-child(7n) .rbc-events-container {
                    display: block !important;
                  }
                  
                  /* Today's Saturday styling - simplified */
                  .rbc-month-view .rbc-date-cell.rbc-today:nth-child(7n) {
                    background: transparent !important;
                    border: none !important;
                  }
                  
                  /* Modern styling for all calendar cells */
                  .rbc-month-view .rbc-date-cell {
                    transition: all 0.2s ease !important;
                  }
                  
                  /* Hover effects for Saturday cells - simplified */
                  .rbc-month-view .rbc-date-cell:nth-child(7n):hover {
                    background: transparent !important;
                  }
                  
                  /* General cell hover effects */
                  ...
                  .rbc-month-view .rbc-date-cell:nth-child(7n) .rbc-event {
        ...
        <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {Object.entries({
              class: { color: '#22C55E', icon: BookOpen, label: 'Classes' },
              ...
              cancelled_class: { color: '#64748B', icon: X, label: 'Cancelled' }
            }).map(([type, info]) => {
          ...
          <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700/50 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleSaveSettings} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select
                        value={settingsForm.default_view}
                        onChange={(e) => handleSettingsChange('default_view', e.target.value)}
                        className="w-full bg-slate-800/60 border border-slate-600/50 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleSettingsChange('time_format', e.target.value)}
                        className="w-full bg-slate-800/60 border border-slate-600/50 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                      >
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        id="weekendVisible"
                        checked={settingsForm.weekend_visible}
                        onChange={(e) => handleSettingsChange('weekend_visible', e.target.checked)}
                        className="w-4 h-4 text-blue-600 bg-slate-800 border-slate-600 rounded focus:ring-blue-500"
                      />
                      <label htmlFor="weekendVisible" className="text-sm font-medium text-slate-300">
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        id="showPastEvents"
                        checked={settingsForm.show_past_events}
                        onChange={(e) => handleSettingsChange('show_past_events', e.target.checked)}
                        className="w-4 h-4 text-blue-600 bg-slate-800 border-slate-600 rounded focus:ring-blue-500"
                      />
                      <label htmlFor="showPastEvents" className="text-sm font-medium text-slate-300">
                        Show past events
                      </label>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        id="autoRefresh"
                        checked={settingsForm.auto_refresh}
                        onChange={(e) => handleSettingsChange('auto_refresh', e.target.checked)}
                        className="w-4 h-4 text-blue-600 bg-slate-800 border-slate-600 rounded focus:ring-blue-500"
                      />
                      <label htmlFor="autoRefresh" className="text-sm font-medium text-slate-300">
                ...
                {/* Event Defaults */}
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">Event Defaults</h3>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">
                      Default Event Duration (minutes)
                    </label>
                    <select
                      value={settingsForm.default_event_duration}
                      onChange={(e) => handleSettingsChange('default_event_duration', parseInt(e.target.value))}
                      className="w-full bg-slate-800/60 border border-slate-600/50 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                    >
