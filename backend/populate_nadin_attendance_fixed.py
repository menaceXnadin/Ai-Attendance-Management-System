...
Uses individual commits to handle PostgreSQL enum types properly
"""
...
def populate_nadin_attendance_fixed():
    """Populate comprehensive attendance data for nadin@gmail.com with individual commits"""
    db = SessionLocal()
    
    try:
        print("📚 POPULATING ATTENDANCE FOR NADIN@GMAIL.COM (FIXED)")
        print("=" * 60)
        ...
        subjects = db.query(Subject).limit(8).all()  # Get 8 subjects for variety
        if not subjects:
        ...
        while current_date <= end_date:
            ...
            # Determine how many classes Nadin has on this day (2-4 classes)
            daily_classes = random.randint(2, 4)
            ...
            for i, subject in enumerate(selected_subjects):
                ...
                hour = 9 + (i * 2)  # Classes at 9, 11, 13, 15
                ...
                # Create attendance record with individual commit
                attendance_record = AttendanceRecord(
                    ...
                    notes=f"Calendar test data for {current_date}"
                )
                
                db.add(attendance_record)
                db.commit()  # Individual commit for each record
                created_records += 1
        ...
        recent_days = db.execute(text("""
            ...
            AND ar.date >= CURRENT_DATE - INTERVAL '10 days'
        ...
        print(f"\n📊 CALENDAR MONTH VIEW (SEPTEMBER 2025):")
        print("-" * 40)
        
        month_data = db.execute(text("""
            SELECT 
                DATE(ar.date) as day,
                COUNT(*) as total_classes,
                SUM(CASE WHEN ar.status = 'present' THEN 1 ELSE 0 END) as present,
                ...
                SUM(CASE WHEN ar.status = 'late' THEN 1 ELSE 0 END) as late,
                CASE 
                    WHEN SUM(CASE WHEN ar.status = 'present' THEN 1 ELSE 0 END) = COUNT(*) THEN 'all-present'
                    WHEN SUM(CASE WHEN ar.status = 'absent' THEN 1 ELSE 0 END) = COUNT(*) THEN 'all-absent'
                    WHEN SUM(CASE WHEN ar.status = 'late' THEN 1 ELSE 0 END) > 0 THEN 'has-late'
                    ELSE 'mixed'
                END as calendar_status
            FROM attendance_records ar
            WHERE ar.student_id = :student_id
            AND EXTRACT(MONTH FROM ar.date) = 9
        ...
        for day in month_data:
            icon = calendar_icons.get(day.calendar_status, '⚪')
            print(f"{icon} {day.day}: {day.total_classes} classes - Status: {day.calendar_status}")
        ...
        print("   🟢 Green = All Present")
        print("   🟡 Yellow = Has Late arrivals")
        print("   🟠 Orange = Mixed attendance")
        print("   🔴 Red = All Absent")
        print("✅ Test month navigation between August/September")
        print("✅ Hover over days to see attendance details")
        print("✅ Click on days to see detailed breakdown")
        print(f"\n🚀 Calendar now has {created_records} rich attendance records for nadin@gmail.com!")
...
if __name__ == "__main__":
    populate_nadin_attendance_fixed()